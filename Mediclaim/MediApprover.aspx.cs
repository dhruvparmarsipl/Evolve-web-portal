...
public partial class MediApprover : System.Web.UI.Page
{
    ...
    {
        //Status is 4 to approve the mediclaim It means Closed
        int res = 0;
        res = DoProcess(4, "");
        if (res != 0)
        {
            MediMain oclsMM = new MediMain();
            string Content = "Your Mediclaim has been Approved by Approver.";
            oclsMM.InsertLogsByApp(hdnSERIALNO.Value, "Close", lblHRName.Text,txtHRComments.Text);
            string Subject = "Mediclaim Approval";
            CommonMail(hdnUserEmail.Value, "4");
            //oclsMM.SendEmail(hdnUserEmail.Value, Content, Subject, lblName.Text);
            ShowMessageAndClose("Mediclaim Approved");
        }
    }
    protected void lnkBtnReject_Click(object sender, EventArgs e)
    {
        //Status is 5 for reject mediclaim it go back to the requestor
        //So that below I made requestor an Owner by passing sapid as ownerid
        int res = 0;
        res = DoProcess(5, hdnLeaveAR_RequestorSAPID.Value);
        MediMain oclsMM = new MediMain();
        if (res != 0)
        {
            string Content = "Your Mediclaim has been rejected";
            string Subject = "Mediclaim Rejected";
            //Sendmail to the user
            CommonMail(hdnUserEmail.Value,"5");
           // oclsMM.SendEmail(hdnUserEmail.Value, Content, Subject, lblName.Text);
            //Insert Log for this Ticket
            oclsMM.InsertLogsByApp(hdnSERIALNO.Value, "Rejected By Mediclaim Admin", lblHRName.Text,txtHRComments.Text);
            ShowMessageAndClose("Mediclaim Rejected");
        }
    }
    ...
    private void BindHeader(string SAPID,string StatusId)
    {
       ...
       // string Owner = Convert.ToString(dt_UInfo.Rows[0]["Owner"]).ToLower();
        if (hdnOwnerSapID.Value.ToLower() == getSAPID(ReturnLoginName()).ToLower())
        {
            if (StatusId == "2")
